...
document.getElementById('exportCsvBtn').onclick=()=>{
  ...
  const a=document.createElement('a'); a.href=url; a.download=`medical_history_${new Date().toISOString().replace(/[:\.\-T]/g,'')}.csv`; a.click(); URL.revokeObjectURL(url);
};
    ...
    btn.onclick=()=>{
      ...
      printWindow.document.write(`
        <html><head><title>Patient Report - ${r.patient_name}</title></head><body style="font-family:Arial;padding:20px;">
          ...
          <h3>Recommendations</h3>
          <ul>
            ${(r.recommendations||[]).map(rec=>`<li>${typeof rec==='string'?rec:(rec.advice||rec.primary_advice||JSON.stringify(rec))}</li>`).join('')}
          </ul>
          <hr>
